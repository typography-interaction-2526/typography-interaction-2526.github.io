...
Students should get in touch with each other after class. The creator of the collection (on the left) should invite their classmate (on the right) [as a <samp>Collaborator</samp>](https://help.are.na/docs/getting-started/channels#adding-collaborators) on the channel, making sure it is [set to <samp>Closed</samp>](https://help.are.na/docs/getting-started/channels#closed-channels). The creator can help explain the theme—beyond what is in the channel description—and can answer any questions around it for its new keeper.
<!-- .before--3 .balance -->
...
You can however add new blocks to the channel and also adjust their order. Each student should now add *at least* 10 of their own items to their collection—following, extending, or expanding the organizing principle of the existing ones. It is yours, now.
...
## Are&period;na as an API

<div class="body">

We’ll now *link* the Are&period;na channel to your static site—using its Application Programming Interface (API) to pull in its content directly. This will utilize JavaScript (JS) to dynamically render the page content from the channel.

With the foundations laid in the static implementation, this connecting-the-dots step will populate the site with its real, live/updating content. Changes on Are&period;na are changes on your site. With real content comes real considerations, and your design *should* evolve from this.
...
There should be no remaining local media or assets in your repo; everything should be coming dynamically from Are&period;na. You are welcome to continue to add blocks to your channel and refine it in the CMS there. And then we will move our focus to refining the design and layout of the site, based on your actual content.
<!-- .before--2 -->
...
Here you will introduce interactivity and dynamism to your project, via your own JavaScript—beyond just the Are&period;na API.
...
### Our [ongoing](/project/3/#our-expectations) requirements
...
- You are required to use images, obviously! (Inline SVG use is up to you.)

- Your project must show *all* the blocks in your channel (all types)—but the *how* of that is entirely up to you.

- We don’t want to see 25 Are&period;na reskins/generic templates—these should be unique to your collection and its theme.
