...
By default, all browsers’ *user-agent styles* have an unfortunate <nobr>default—</nobr>`box-sizing: content-box;`—which means that the padding (and border) exists *outside* the content width or height—so padding (and border) is then an *outset.*
...
Importantly, `position: absolute;` also *removes* the element from the normal document <nobr>flow—</nobr>meaning it takes up *no space* in the page layout.
<!-- .balance -->
