...
Each of these could be full classes themselves. We hope this quick overview (and *copious* linking) can be an introduction if you want to go deeper.
...
<style>
	dl { margin-block-start: calc(3 * var(--typography--line-height)) }
</style>

## Git&thinsp;&thinsp;&thinsp;/GitHub and Collaboration

Our old friend GitHub (and the underlying technology, [Git](https://git-scm.com)&thinsp;) are robust, industry-standard tools for version control and working together on code. But to some extent, we barely scratched the surface! These are some other important Git/GitHub concepts:
<!-- .balance -->

Branches
: Besides our second project, we’ve used Git/GitHub primarily for solo, linear software development—but that is rarely how you work, especially at scale!
	...
	Figma has started to [adopt this concept](https://www.figma.com/best-practices/branching-in-figma/best-practices-when-using-branches/), too! <!-- .secondary -->
...
PR&thinsp;s (Pull Requests)
: When you are working on big software projects/repos with multiple people—and when you don’t want to break `main` with hasty merges—you *push* your code up to GitHub and [open a *pull request*](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request) (everyone says *P&thinsp;-&thinsp;R*). This shows all your combined commits together, and allows your team [to review](https://www.youtube.com/watch?v=rgbCcBNZcdQ&pp=ygUOInB1bGwgcmVxdWVzdCI%3D) and approve your whole batch of work before you merge into `main`.
...
: A popular way to structure software project management. Many product teams will use [*Agile*](https://www.atlassian.com/agile) methodology as a way of prioritizing tasks—working in so-called *sprints* or *cycles* (often two weeks) to deliver features incrementally rather than all at once.
	...
	[Many](https://www.atlassian.com/) [tools](https://linear.app/) [are](https://clickup.com) built around this mindset! It’s a bit like our roadmapping exercise.<!-- .secondary -->
...
: Very often, products have to make trade-offs between design or code quality to meet deadlines, or to handle [*scope creep*](https://asana.com/resources/what-is-scope-creep). These decisions sometimes come at the expense of either side—sacrificing the most nuanced design, or giving up on the most pristine, sturdy code. Often the short-term wins over the long-term.
	...
	In this lens, it might be helpful to think of “AI”&thinsp;/&thinsp;LLMs as credit cards. They’ll let you spend quickly, but you’ll have to pay the debt eventually! <!-- .secondary -->

	Richard Scarry [understood](https://miro.medium.com/v2/resize:fit:2000/format:webp/0*iFaxYkoSpxX5gOPE.jpeg). <!-- .secondary -->
...
: [*A11y*](https://www.a11yproject.com/) is a catch-all term for [accessibility](https://developer.mozilla.org/en-US/docs/Web/Accessibility). This is shorthand for ensuring that your product is as inclusive as possible—for folks with different disabilities—often estimated to be [around 15%](https://www.un.org/development/desa/disabilities/resources/factsheet-on-persons-with-disabilities.html) of the world’s population.
	...
	You might also see this kind of abbreviation (numeronym) for [*i18n*](https://en.wikipedia.org/wiki/Internationalization_and_localization), for *internationalization*. <!-- .secondary -->
...
: These are platform-specific, commonly-used assistive technologies: Apple’s [*VoiceOver*](https://www.apple.com/accessibility/vision/), Google’s (Android) [*TalkBack*](https://support.google.com/accessibility/android/topic/3529932?hl=en&ref_topic=9078845&sjid=8833292709650514472-NA), and Microsoft’s [*Narrator*](https://support.microsoft.com/en-us/windows/complete-guide-to-narrator-e4397a0d-ef4f-b386-d8ae-c172f109bdb1).
	...
	[Many disabled folks](https://webaim.org/projects/screenreadersurvey9/) use other screen-readers (namely [JAWS](https://www.freedomscientific.com/products/software/jaws/) and [NVDA](https://www.nvaccess.org/download/)), but these are a good, built-in start. <!-- .secondary -->
...
Multi-Page Apps (MPA&thinsp;s) vs. Single-Page Apps <nobr>(SPA&thinsp;s)</nobr> <!-- .balance -->
: For most of you, everything you’ve done this year could be called a *multi-page app*—meaning just that your websites (products) have been comprised of discrete, separate HTMLpages. Remember that the web started as simple *MPA*, though the term didn’t exist yet!
	...
	The line here is getting even blurrier—when you hear things like [*server-side rendering (SSR)*](https://www.sanity.io/glossary/server-side-rendering) and even [*partial hydration*](https://ajcwebdev.com/what-is-partial-hydration/). <!-- .secondary -->
...
: As you may have noticed, writing CSS can be repetitive and tiresome!
	...
	This very course site is the first thing we’ve built in more than a *decade* that doesn’t use (because it didn’t need) SASS! <!-- .secondary -->
...
: On the other end of the spectrum—an alternative approach to CSS naming—following a [*utility class*](https://css-irl.info/a-year-of-utility-classes/) methodology.
	...
	Your instructors wrote our [own <em>atomic</em> CSS library](https://uxdesign.cc/tools-not-rules-9daef895aab7#3234), baking-in/formalizing MoMA’s brand, that we called [Sol](https://www.moma.org/artists/3528). It seemed like a good idea at the time!<!-- .secondary -->
