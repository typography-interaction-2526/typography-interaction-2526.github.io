...
After “shopping” among the various [recreations](https://aresluna.org/the-hardest-working-font-in-manhattan/recreations/), we settled on [N. E. Bartgis’](https://github.com/drdnar) cut [*GortonDigital*](https://github.com/drdnar/GortonDigital) for its charm (and lowercase). Like many *Gortons*, it lacks an italic; we fake ours as you would with [a pantograph](https://en.wikipedia.org/wiki/Pantograph#:~:text=condense%2C%20extend%2C%20and-,slant%20the%20design,-%28mathematically%2C%20these%20are), with a simple [`skew()`](https://typography-interaction-2526.github.io/week/2/#reading-discussion:~:text=our%20first%20unit%2C-,Type,Web,-%3A). (Students: *do as we say, not as we do!*&#x202F;) This only felt right for lowercase though, so `<em>` get rendered [with quotes](https://typography-interaction-2526.github.io/syllabus/#unit-1-type-and-the-web:~:text=UNIT%C2%A0N%C2%BA%C2%A01%3A-,TYPE%20AND%20THE%20WEB,-WEEKS%20N%C2%BA%C2%A01) when set all-caps. We even fudge some extra weight with [`-webkit-text-stroke-width`](https://typography-interaction-2526.github.io/syllabus/#readings:~:text=AND%20THE%20WEB-,WEEKS%20N%C2%BA%C2%A01%E2%80%936,-We%20will%20focus) where we needed more range—as if loading a thicker cutting bit, or tracing the tool path over again.

As we used *GortonDigital* over the year, we’ve also added some additional characters and [tweaked it to taste](https://github.com/drdnar/GortonDigital/pull/8). This all feels appropriate to this vernacular, ever-evolving typeface.
...
- Our *many* [code example blocks](https://typography-interaction-2526.github.io/topic/javascript/#intersection) are put together with a few tricks:
	...
	- We also render a [preview page](https://typography-interaction-2526.github.io/topic/javascript/intersection/preview/?active=script.js&width=75%) out, which wraps the examples in our custom [`ti-preview` web component](https://github.com/typography-interaction-2425/ti-preview).
		...
		**This enables in-browser editing, with a live preview, via [CodeMirror](https://codemirror.net/)! These have been *really* helpful in-class, for quick little demos.**
