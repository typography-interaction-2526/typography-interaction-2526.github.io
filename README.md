...
You’ll need some familiarity with [the command line](https://developer.mozilla.org/en-US/docs/Learn_web_development/Getting_started/Environment_setup/Command_line), here! We’re using [`pnpm`](https://pnpm.io) as our package manager, to pull in our [build dependencies](package.json)—the various libraries the site uses. Run `pnpm install` once in the repo directory to grab what we need!
